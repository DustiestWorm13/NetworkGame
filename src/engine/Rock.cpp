...
/**
* /brief this is where all data values are set for the rock
* 
* health is set to 3 for the rock splitting
* mThrustDir is set to 0.3 for the speed of the rock
* 
* 
* 
*/
...
void Rock::ProcessInput( float inDeltaTime, const InputState& inInputState )
{
	//process our input....
...
void Rock::AdjustVelocityByThrust( float inDeltaTime )
{
...
void Rock::SimulateMovement( float inDeltaTime )
{
	//simulate us...
	...
	for( auto goIt = World::sInstance->GetGameObjects().begin(), end = World::sInstance->GetGameObjects().end(); goIt != end; ++goIt )
	{
		...
		if( target != this && !target->DoesWantToDie() )
		{
			...
			if( distSq < ( collisionDist * collisionDist ) )
			{
				...
				if( target->HandleCollisionWithRock( this ) )
				{
					...
					if( otherRock )
					{
						relVel -= otherRock->mVelocity;
					}
					Bullet* bullet = dynamic_cast<Bullet*>(target);
					if (bullet)
					{
						SetDoesWantToDie(true);
						bullet->SetDoesWantToDie(true);
					}
...
void Rock::ProcessCollisionsWithScreenWalls()
{
	...
	if( ( y + radius ) >= (HALF_WORLD_HEIGHT + 1.0f) && vy > 0 )
	{
		SetLocation(GetLocation() - Vector3(0.0f, 7.2f, 0.0f));
	}
	else if( y <= ( -HALF_WORLD_HEIGHT - radius - 8.0f ) && vy < 0 )
	{
		SetLocation(GetLocation() + Vector3(0.0f, 7.2f, 0.0f));
	}

	if( ( x + radius ) >= (HALF_WORLD_WIDTH + 1.0f) && vx > 0 )
	{
		SetLocation(GetLocation() - Vector3(12.8f, 0.0f, 0.0f));
	}
	else if(  x <= ( -HALF_WORLD_WIDTH - radius - 1.0f ) && vx < 0 )
	{
		SetLocation(GetLocation() + Vector3(12.8f, 0.0f, 0.0f));
	}
}
